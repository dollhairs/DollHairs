...
uint256 hashGenesisBlock("0x0a8269fec6aa4d9462ce3be9bc1b3cef70c4f3b5995a8135d3488948ddfdc78a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DollHairs: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Euphoria Incoming:\n";
...
{
    // DollHairs: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	//Block Reward is cut in half every 3 months
	nSubsidy >>= (nHeight / 162000); //DollHairs

    return nSubsidy + nFees;
}

static int64 nTargetTimespan =  45; // change difficulty every 10 minutes with old algos
static int64 nTargetSpacing = 45; // new block every minute
...
{
    static const int64 BlocksTargetSpacing = 45; // 1 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DollHairs: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // DollHairs: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "DollHairs launches";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xb3417148359438505448970316bbc0e54099faedcf60b6b80153d66560d62714"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DollHairsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DollHairsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DollHairsMiner(CWallet *pwallet)
{
    printf("DollHairsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dollhairs-miner");
    ...
    try { loop {
        ...
        printf("Running DollHairsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DollHairsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DollHairsMiner, pwallet));
}
